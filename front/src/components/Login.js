...
function Login() {
    ...
    return (
        <form className='container mx-auto bg-white flex flex-col min-w-min max-w-screen-sm h-fit items-center rounded-lg mt-10 shadow-lg'>
            ...
            <input id='login' className='bg-gray-50 border text-gray-800 text-xs w-11/12 h-10 p-2 rounded focus:outline-none focus:bg-gray-100' autoComplete='username' type='text' onChange={(e) => e.preventDefault(updateMail(e.target.value))}/>
            <label htmlFor='password' className='mt-4 ml-8 mr-auto text-sm text-gray-800 font-medium'>Password</label>
            <input id='password' className='bg-gray-50 border text-gray-800 text-xs w-11/12 h-10 p-2 rounded focus:outline-none focus:bg-gray-100' type='password' autoComplete='current-password' onChange={(e) => e.preventDefault(updatePass(e.target.value))}/>
